...
class Checkers:
    ...
    def CompTurn(s):
        ...
        s.log_file = open('debug_log.txt', 'w')
        
        move_eval, best_move = s.negamax(
            depth = s.comp_depth, 
            alpha = float('-inf'), 
            beta = float('inf'), 
            color = s.pTurn, 
    ...
    def evaluate_tiles(s, tiles, color):
        score = 0
        for i in range(s.BoardDimension):
            for j in range(s.BoardDimension):
                if tiles[i][j].isPiece:
                    king_row = 0 if tiles[i][j].isBlack else 7
                    piece_score = 1 if tiles[i][j].isPawn else 5
                    #piece_score = 3 * (8 - abs(j - king_row)) / 8.0 if tiles[i][j].isPawn else 10  # Assign score to piece type
                    if tiles[i][j].pieceColour == color:
                        score += piece_score
                    else:
                        score -= piece_score
        return score
    ...
    def negamax(s, depth, alpha, beta, color, tiles):
        valid_moves = s.valid_moves(tiles, color) # Obtain all valid moves for the evaluating color's pieces
        
        # If depth is 0, evaluate the position
        if not depth:
            evaluation = s.evaluate_tiles(tiles, color)
            # s.debug_log(
            #     "Depth: {depth}, Color: {color}, Evaluation: {evaluation}"
            #     .format(
            #         depth=depth,
            #         color=color,
        ...
        if valid_moves:
            ...
            for move in valid_moves:
                ...
                cur_eval, continuation_move = s.negamax(
                    depth = depth - 1, 
                    alpha = -beta, 
                    beta = -alpha, 
                    color = s.opposite(color), 
                    tiles = new_tiles
                )
                
                cur_eval *= -1
                # s.debug_log(
                #     "Depth: {depth}, Move: {move}, Evaluation: {cur_eval}, Alpha: {alpha}, Beta: {beta}, Best Move: {best_move}"
                #     .format(
                #         depth=depth,
                #         move=move,
                #         cur_eval=cur_eval,
                ...
                if depth == s.comp_depth:
                    print(
                        'Evaluated Move: {move} ({cur_eval}) Best Opponent Continuation: {continuation_move}'
                        .format(
                            move=move, 
                            cur_eval=cur_eval, 
                ...
                if cur_eval > max_eval:
                    ...
                    best_move = move # Reassign the best move
                ...
                if alpha >= beta:
                    # s.debug_log(
                    #     "Alpha-beta cutoff at Depth: {depth}, Move: {move}, Alpha: {alpha}, Beta: {beta}"
                    #     .format(
                    #         depth=depth,
                    #         move=move, 
                    #         alpha=alpha, 
        ...
        # s.debug_log(
        #     "Depth: {depth}, Best Move: {best_move}, Max Evaluation: {max_eval}, Alpha: {alpha}, Beta: {beta}"
        #     .format(
        #         depth=depth,
        #         best_move=best_move,
        #         max_eval=max_eval,
